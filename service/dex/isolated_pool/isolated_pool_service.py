...
class IsolatedPoolService:
    ...
    async def register_pool(cls, event: SubstrateEvent[XYKPoolCreatedPayload]):
        ...
        pool = await Pool.get_or_none(account=event.payload['pool'])
        if pool:
            assert pool.shares == '0'
            assert pool._saved_in_db is True
        else:
            pool = Pool(account=event.payload['pool'])

        pool.dex_key = DexKey.IsolatedPool
        pool.dex_pool_id = event.payload['pool']
        pool.lp_token_id = event.payload['share_token']
        pool.shares = event.payload['initial_shares_amount']  # type: ignore[assignment]
        await pool.save(
            # NOTE: Special case: pool was destroyed in the same transaction
            force_update=event.payload['pool'] in destroyed_pools,
        )
